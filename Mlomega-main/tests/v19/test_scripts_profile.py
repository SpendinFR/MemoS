...
def test_livephone_launcher_runs_strict_readiness_before_server():
    text = Path('scripts/RUN_MLOMEGA_V19.ps1').read_text(encoding='utf-8')
    check = text.index('check_phoneonly_readiness.py')
    server = text.index('sessionhub_http.py')
    assert check < server
