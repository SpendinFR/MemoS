...
def test_nested_turn_reference_is_cited_and_prose_only_claim_is_not_canonical(tmp_path, monkeypatch):
    db_path = _fixture_db(Path(tmp_path), monkeypatch)
    from mlomega_audio_elite.brain2_complete_v13 import ENGINE_SCHEMAS
    ...
    with connect(db_path) as con:
        output = {
            "situation": {
                "situation_type": "conversation", "life_domain": "personal",
                "participants": [], "main_person": None, "targets": [],
                "place_explicit": None, "place_inferred": None,
                "social_context": "informal", "power_balance": "unknown",
                "stakes": "low", "constraints": [{"turn_id": "t1", "value": "demain"}],
            },
            "resolved_references": [{"expression": "demain", "referent": "date suivante"}],
            "missing_context": [],
            "evidence": [{"text": "Le locuteur parle d'un rendez-vous."}],
            "counter_evidence": [], "confidence": 0.8,
        }
        record_engine_output(
            con, person_id="me", conversation_id="conv-i2", episode_id="ep-i2",
            engine_name="context_resolver", output=output,
            schema=ENGINE_SCHEMAS["context_resolver"],
            applicability_reason="quality_test",
        )
        rows = con.execute(
            "SELECT fact_type,evidence_status FROM brain2_shared_facts_v19 ORDER BY fact_type"
        ).fetchall()
        # The situation is traceable through its nested turn_id. The prose-only
        # resolved reference remains losslessly in the engine section, not facts.
        assert [tuple(row) for row in rows] == [("situation", "cited")]
        assert con.execute(
            "SELECT COUNT(*) FROM brain2_shared_engine_sections_v19 WHERE engine_name='context_resolver'"
        ).fetchone()[0] == 1


def test_unique_validated_subtheme_recovers_scope_without_fuzzy_promotion(tmp_path, monkeypatch):
    db_path = _fixture_db(Path(tmp_path), monkeypatch)
    from mlomega_audio_elite.brain2_complete_v13 import ENGINE_SCHEMAS
    from mlomega_audio_elite.brain2_shared_facts_v19 import record_engine_output
    from mlomega_audio_elite.db import connect
    ...
    with connect(db_path) as con:
        con.execute(
            """INSERT INTO episode_subthemes_v19(
                   ...
                   start_turn_id,end_turn_id,participants_json,confidence,
                   metadata_json,created_at,updated_at
               ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            ("st-karim", "ep-i2", 0, "relationship", "Rendez-vous avec Karim",
             "Karim avertit le propriétaire au sujet du rendez-vous de demain.",
             "t1", "t1", "[]", 0.9, "{}", now, now),
        )
        con.execute(
            """INSERT INTO episode_subthemes_v19(
            ...
            ("st-netflix", "ep-i2", 1, "leisure", "Choix Netflix",
             "Discussion sur un documentaire à regarder sur Netflix.",
             "t1", "t1", "[]", 0.8, "{}", now, now),
        )
        con.execute(
            """INSERT INTO episode_subtheme_evidence_v19(
                   subtheme_evidence_id,subtheme_id,turn_id,evidence_role,
                   confidence,created_at
               ) VALUES(?,?,?,?,?,?)""",
            ("ste-karim", "st-karim", "t1", "primary_citation", 0.9, now),
        )
        output = {
            "relationship_updates": [
                {"other_person_id": "person_karim", "relationship_type": "ami",
                 "trust_delta": 0.0, "tension_delta": 0.0,
                 "common_trigger": "Karim avertit le propriétaire du rendez-vous demain",
                 "repair_action": "", "confidence": 0.8},
                {"other_person_id": "person_unknown", "relationship_type": "inconnu",
                 "trust_delta": 0.0, "tension_delta": 0.0,
                 "common_trigger": "échange personnel important",
                 "repair_action": "", "confidence": 0.7},
            ],
            "social_roles": [], "conflict_loops": [],
            "evidence": ["Karim parle du rendez-vous de demain."],
            "counter_evidence": [], "confidence": 0.8,
        }
        record_engine_output(
            con, person_id="me", conversation_id="conv-i2", episode_id="ep-i2",
            engine_name="social_model_engine", output=output,
            schema=ENGINE_SCHEMAS["social_model_engine"],
            applicability_reason="quality_test",
        )
        facts = con.execute(
            "SELECT payload_json FROM brain2_shared_facts_v19 WHERE source_engine='social_model_engine'"
        ).fetchall()
        assert len(facts) == 1
        assert "person_karim" in facts[0][0]
        citation = con.execute(
            "SELECT turn_id,citation_role FROM brain2_shared_fact_evidence_v19"
        ).fetchone()
        assert tuple(citation) == ("t1", "validated_subtheme_scope")
