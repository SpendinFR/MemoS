...
namespace MLOmega.XR.UI
{
    ...
    public sealed class WorldCreatorLabShell : MonoBehaviour
    {
        ...
        private IEnumerator Start()
        {
            ...
            _dictation = gameObject.AddComponent<XrLabVoiceDictation>();
        ...
        {
            // The Web VR browser is an eleventh first-class app. Keep a compact
            // 4-4-3 visionOS grid instead of stacking it over the validated apps.
            ...
            {
                MoveExistingDockGroup(existing[0], new Vector2(-225f, 150f));
                MoveExistingDockGroup(existing[1], new Vector2(-75f, 150f));
        ...
        private void BuildKeyboard()
        {
            ...
            MakeButton(
                ...
                ReceiveClearAll);
            Button micButton = MakeButton(
                _keyboardRect,
                "MIC",
                new Vector2(280f, -175f),
                new Vector2(112f, 62f),
                StartDictation);
            _micLabel = micButton.GetComponentInChildren<TextMeshProUGUI>();
            MakeButton(
                _keyboardRect,
                "ENTRÉE",
        ...
        private void StartDictation()
        {
            ...
            {
                _keyboardPreview.text = "Traitement…";
                _dictation.StopListening();
                return;
            }
            _keyboardPreview.text = "Écoute…";
            _dictation.StartListening("fr-FR");
        }
        ...
        private void OnDictationResult(string text)
        {
            ...
            Debug.Log("[XrLab] dictation inserted: " + text.Trim());
        }

        private void OnDictationError(string message)
        {
            Debug.LogWarning("[XrLab] dictation: " + message);
            if (_keyboardPreview != null)
                _keyboardPreview.text = "Dictée : " + message;
        }

        private void OnDictationListeningChanged(bool listening)
        {
            if (_micLabel != null) _micLabel.text = listening ? "STOP" : "MIC";
            if (!listening && _keyboardPreview != null &&
                _keyboardPreview.text == "Écoute…")
                _keyboardPreview.text = "Traitement…";
        }
    ...
    public sealed class XrLabVoiceDictation : MonoBehaviour
    {
        ...
        public void StartListening(string language)
        {
                    ...
                    UnityEngine.Android.Permission.Microphone))
            {
                UnityEngine.Android.Permission.RequestUserPermission(
                    UnityEngine.Android.Permission.Microphone);
                RaiseError("autorise le micro puis retouche MIC");
                return;
            }
            SetListening(true);
            using var player = new AndroidJavaClass("com.unity3d.player.UnityPlayer");
            using AndroidJavaObject dispatchActivity =
                player.GetStatic<AndroidJavaObject>("currentActivity");
            dispatchActivity.Call("runOnUiThread", new AndroidJavaRunnable(() =>
            {
                try
                {
                    using var uiPlayer = new AndroidJavaClass(
                        "com.unity3d.player.UnityPlayer");
                    using AndroidJavaObject activity =
                        uiPlayer.GetStatic<AndroidJavaObject>("currentActivity");
                    if (_recognizer == null)
                    {
                        using var recognizerClass = new AndroidJavaClass(
                            "android.speech.SpeechRecognizer");
                        _recognizer = recognizerClass.CallStatic<AndroidJavaObject>(
                            "createSpeechRecognizer",
                            activity);
                        _listener = new RecognitionListener(this);
                ...
                {
                    Enqueue(() => RaiseError(exception.Message));
                }
            }));
#else
            RaiseError("dictée disponible sur Android uniquement");
#endif
        }
        ...
        {
#if UNITY_ANDROID && !UNITY_EDITOR
            if (!IsListening || _recognizer == null) return;
            using var player = new AndroidJavaClass("com.unity3d.player.UnityPlayer");
            using AndroidJavaObject activity =
                player.GetStatic<AndroidJavaObject>("currentActivity");
            activity.Call("runOnUiThread", new AndroidJavaRunnable(() =>
            {
                try
                {
                    _recognizer?.Call("stopListening");
                }
                catch (Exception exception)
                {
                    Enqueue(() => RaiseError(exception.Message));
        ...
        {
            lock (_gate) _mainThread.Enqueue(action);
        }

        private void RaiseResult(string text)
        {
            SetListening(false);
            Debug.Log("[XrLab] dictation result: " + text);
            Result?.Invoke(text);
        }
        ...
        {
            SetListening(false);
            Error?.Invoke(FriendlyError(message));
        }
        ...
        private static string FriendlyError(string message)
        {
            ...
            {
                "code 5" => "reconnaissance occupée, retouche MIC",
                "code 7" => "aucune voix reconnue",
                _ => message,
            };
        }
        ...
        {
            private readonly XrLabVoiceDictation _owner;

            public RecognitionListener(XrLabVoiceDictation owner)
            ...
            {
                _owner.Enqueue(() => _owner.RaiseError("code " + error));
            }
            ...
            public void onResults(AndroidJavaObject bundle)
            {
                try
                {
                    ...
                    using AndroidJavaObject values =
                        bundle.Call<AndroidJavaObject>("getStringArrayList", key);
                    string text = values == null
                        ? string.Empty
                        : values.Call<string>("get", 0);
                    _owner.Enqueue(() => _owner.RaiseResult(text));
                }
                catch (Exception exception)
                {
                    _owner.Enqueue(() => _owner.RaiseError(exception.Message));
                }
            }
        }
    }
