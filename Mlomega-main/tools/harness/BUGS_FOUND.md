...
## OBS-51 — OCR refusé dès qu'un autre modèle occupait le GPU (CORRIGÉ — Gate B)

`GpuArbiter.request` comparait la VRAM totale utilisée au budget du job demandé. Pour OCR,
`used_mb > 768` suffisait donc à refuser le travail, même avec plusieurs Gio libres.
L'admission calcule maintenant la VRAM projetée (`used + coût_job`) contre le plafond de
la carte. Une vraie frame du scénario a ensuite révélé une seconde limite : RapidOCR peut
s'abstenir sur un écran oblique/compressé. Un fallback uniquement sur demande utilise
`qwen3-vl:4b`, JSON contraint, et reste `probable`; preuve réelle 7,05 s et texte non vide.

## OBS-52 — ChangeAttention n'avait jamais de zone active (CORRIGÉ — Gate B)

`PoseKeyframeMap.active_zone()` existait, mais `WorldBrain.ingest_scene_delta` ne copiait
pas sa valeur dans `SceneSession.active_zone`. Les changements étaient présents tandis
que ChangeAttention retournait systématiquement zéro cue/reentry. La propagation produit
est branchée, respecte les zones suspendues et conserve la dernière zone sur erreur
transitoire; test de snapshot zone-1→zone-2.

## OBS-53 — « retiens demain/rendez-vous » enrôlait de fausses personnes (CORRIGÉ — Gate B)

Le regex court de `EnrollmentWatcher` n'était pas ancré et absorbait le premier mot de
tout souvenir. Le run réel a ainsi compté deux enrôlements (« demain », « rendez-vous »).
La forme courte exige désormais l'utterance complète; les deux phrases exactes du
scénario vont au handler `remember_fact` et à ConversationBridge avec un event_id stable.

## OBS-54 — « traduis le texte » était traité sans traduire (CORRIGÉ EN CODE/UNITY)

IntentRouter posait `translate=true` mais transformait la requête en `what_is`; VisionRT
ignorait le flag. Le nouveau flux demande OCR, puis envoie le texte par `translate_text`
au `DeviceCommandHandler`, qui le remet au `TranslateBridge` offline Android. Le PC ne
réimplémente pas la traduction. Test Unity E33 10/10; exécution native Kotlin/modèles à
confirmer sur S25.

## OBS-55 — Une ancienne tentative Deep Vision bloquait le manifeste réparé (CORRIGÉ)

Le manifeste additionnait les runs de la journée : un vieux 16/9/0 pouvait rendre faux le
run autoritaire 16/16/16. Il valide maintenant le `run_id` explicitement référencé par le
post-stop courant. La tentative échouée reste auditable. Preuve : CloseDay
`run_v18_66f56f15fc154e948827d4f4d53e9236` completed, manifests complets, aucun blocker.

## OBS-56 — Le harnais prouvait le routage, pas l'exécution (CORRIGÉ)

Les 781 downlinks du premier passage n'étaient ni typés ni conservés. Le pipeline émet
maintenant `command_execution_trace` par segment et le fake device garde types, traces et
