...
- [ ] **Fix keyframes hors temp** : `visionrt.default_keyframe_sink` écrit dans un dossier média géré et persistant (sous `MLOMEGA_MEDIA`/`storage/media/keyframes/AAAA-MM-JJ/`), pas `tempfile` ; `vision_frames.image_path` pointe ce chemin stable. Rétro-compat : chemins existants lus tels quels.
- [ ] **Transcodage audio post-close-day** : après re-transcription HQ (stage nocturne terminé), transcoder les WAV `audio_archive` en Opus (~÷10, 4 Go/j → ~400 Mo/j) — qualité preuve orale + re-traitement futur préservée ; met à jour le chemin/déplace, garde le sha d'origine en métadonnée. Réversible/désactivable par config.
- [ ] **Purge des non-référencés âgés** : supprimer UNIQUEMENT les médias (keyframes/clips/audio) qu'AUCUNE preuve ne cite (pas dans evidence_refs d'hypothèse/événement/mémoire) ET plus vieux que `retention_days` (défaut généreux, ex. 90 j). Le référencé n'est JAMAIS supprimé.
- [ ] **Budget global 100 Go avec éviction protégée** : quand l'usage média dépasse le quota, évincer le plus ancien NON-référencé d'abord ; ne jamais toucher au référencé ; si tout le reste est référencé et qu'on dépasse → WARN DOCTOR, pas de suppression. Câbler enfin `EvidenceStore`/`purge` (aujourd'hui scaffold non instancié) OU un module de rétention dédié qui opère sur les tables réelles (`raw_assets`, `vision_frames`, `visual_evidence_assets_v19`, chunks audio_archive).
- [ ] **`storage_quota` dans les profils** : ajouter le bloc à `configs/profiles/rtx3070.yaml` (+ `user_profile.yaml` documenté) — `total_gb: 100`, `warn_gb`, `retention_days`, `transcode_audio: true` ; DOCTOR `-Quota` lit ces valeurs (aujourd'hui absentes → défauts codés en dur) et la rétention aussi.
- [ ] Tests ciblés : périodes longitudinales (fait), sélection des non-référencés, éviction protège le référencé, transcode réversible, DOCTOR lit le quota profil. ADR DECISIONS.md.
