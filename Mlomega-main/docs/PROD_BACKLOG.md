...
1. [ ] Choisir l'état production LLM : soit arrêter le serveur P1 orphelin et démarrer Ollama, soit déclarer `MLOMEGA_LLM_BACKEND=llamacpp`, alias exact et `MLOMEGA_OLLAMA_CONTEXT_POSTSTOP=24576`; Ollama reste requis pour le live/VLM. Relancer seulement `check_phoneonly_readiness.py --deep` jusqu'à `ready=true`.
2. [ ] Exécuter `test_phoneonly_runtime.py` dans `.venv-live` et les tests mémoire E64/I dans `.venv`; aucun nouveau CloseDay long pour cette étape.
3. [ ] Fermer les vérités produit encore ouvertes I0.1–I0.4 : Deep Vision réel `analyzed>0`, plafonds ASR/épistémologie et manifeste de capacités. I0.6 (« où sont mes lunettes ? ») reste un branchement séparé à prouver.
4. [ ] Reprendre I4 (sélection/cache VLM sans perdre d'événement), puis I7 Gate B cinq minutes. Mesurer appels/tokens/temps et dashboard sur un run frais; ensuite seulement projeter une heure/huit heures. Aucun résultat actuel n'autorise encore la promesse `1 h capture <= 1 h consolidation`.
