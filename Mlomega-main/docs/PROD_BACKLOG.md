...
> - **Run 014448 (dernier état)** : P1 démarré par la frontière (ready en 6 s), UN appel segmentation (~10 s) → **`segmentation_not_lossless`** : la segmentation 9B n'a pas couvert les 137 tours du flux Groq (fillers omis en fin). Constats pour arbitrage Codex : (1) le chemin segmentation SINGLE-CALL n'a NI échelle de retry NI persistance du rejet (contrairement au chemin fenêtré et à `brain2_conversation_detail`) — une tentative puis blocked, sortie brute perdue ; (2) le flux de tours Groq (137 tours dont fillers) diffère de WhisperX local sur lequel ce chemin passait ; interdits maintenus (pas d'affectation heuristique des tours omis). (3) Live de ce run : 1 effet device non prouvé (`device:11000:11900`) + `ui_intents_delivered=0` alors que 012416 faisait 13/13 + 2 — variabilité live à élucider. Budget jour cumulé ≈ 0,062 €.
