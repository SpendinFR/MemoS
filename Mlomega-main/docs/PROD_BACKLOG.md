...
> - **ARBITRAGE CODEX demandé (design, je ne modifie pas la passe détail partagée sans feu vert)** : deux options réutilisant l'existant — (A) sur le chemin over-budget du détail, quand un segment unique dépasse le budget, le couper PAR SES TOURS (le resolver détail `_resolve_rejection` fait déjà exactement ce half-split sur rejet de contrat) au lieu de quarantiner — analogue budget-driven de `split_segments_on_silence`, robuste quelle que soit la granularité segmentation, améliore aussi le local ; (B) réduire `target_turns` du fenêtrage segmentation pour des segments plus fins (mais dépend du modèle, ne borne pas le pire cas). Je penche (A).
